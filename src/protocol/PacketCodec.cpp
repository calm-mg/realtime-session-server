...
std::vector<std::uint8_t> PacketCodec::encode(
  ...
  if (total_size > kMaxPacketSize ||
      total_size > std::numeric_limits<std::uint16_t>::max()) {
...
std::vector<std::uint8_t> PacketCodec::encode(PacketType type,
...
std::pair<float, float> PacketCodec::decodePosition(
