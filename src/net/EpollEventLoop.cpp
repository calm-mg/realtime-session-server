...
EpollEventLoop::EpollEventLoop() : epoll_fd_(::epoll_create1(EPOLL_CLOEXEC)) {
  if (epoll_fd_ < 0) {
...
std::vector<epoll_event> EpollEventLoop::wait(int timeout_ms,
                                              int max_events) const {
  std::vector<epoll_event> events(static_cast<std::size_t>(max_events));
  const auto count =
      ::epoll_wait(epoll_fd_, events.data(), max_events, timeout_ms);
  if (count < 0) {
