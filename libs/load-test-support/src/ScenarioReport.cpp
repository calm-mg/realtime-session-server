...
std::string formatRunResult(std::size_t run, ScenarioKind kind,
  ...
  output << " elapsed_sec=" << elapsed_seconds
         ...
         << " p99_ms=" << latency.p99_us / 1000.0 << " server_stats="
         << (result.server_stats_available ? "available" : "unavailable");
  if (result.server_stats_available) {
    output << " read_pauses=" << result.overload.read_pauses
           ...
           << result.overload.max_inbound_queue_size
           ...
           << result.overload.disconnect_peer_closed
           << " disconnect_socket_error="
           << result.overload.disconnect_socket_error
           << " disconnect_protocol_error="
           << result.overload.disconnect_protocol_error
           << " disconnect_idle_timeout="
           << result.overload.disconnect_idle_timeout
  ...
  output << " target=" << (options.host.empty() ? "embedded" : "external")
         << " host=" << (options.host.empty() ? "127.0.0.1" : options.host)
         << " port=" << options.port;
  return output.str();
}
