...
여러 TCP 연결의 요청 순서, 과부하 제어와 종료 처리를 구현하고 검증하는
프로젝트입니다. 채팅방과 위치 전송은 이 서버 구조를 실행하고 시험하는
기능입니다. 서버는 한 I/O 스레드에서 소켓을 관리하고, 별도의 worker
스레드에서 명령을 처리합니다.
...
`server_started`가 기록된 다음 시작하며, thread 생성에 실패해도 서버는
계속 실행되고 주기 출력만 비활성화됩니다.
...
콘솔과 Qt 클라이언트는 이름과 방 이름을 앞뒤 ASCII 공백 제거 후 최대 32
UTF-8 byte로 제한하고, 채팅은 공백을 보존한 채 최대 1291 UTF-8 byte까지
전송합니다. `%`, `|`, `=`가 포함된 이름·방·채팅도 서버 응답과 broadcast에서
원래 문자열로 복원됩니다. 잘못된 UTF-8이나 상한 초과 입력은 송신 전에
안내하며 현재 연결을 유지합니다.
...
`7777`입니다. 문자열 byte 상한과 예약문자 지원은 콘솔 클라이언트와
동일합니다.
...
채팅·시스템·오류 로그는 합계 최근 1,000개를 보존하며 오래된 항목부터
제거합니다. 송신 대기 데이터는 패킷 헤더와 Qt 소켓 내부 버퍼를 포함해
1 MiB로 제한합니다. 한도를 넘는 패킷은 대기열에 넣지 않고 오류를 표시하며,
연결과 채팅 입력을 유지하므로 대기 데이터가 전송된 뒤 다시 시도할 수 있습니다.
