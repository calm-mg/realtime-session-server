...
집합을 분리한 외부 대상 측정도 수행했습니다. 외부 반복의 소켓 오류는 같은
조건에서 종료 패킷으로 재현했고 부하 클라이언트의 정리를 수정했습니다.
지연 분석에서는 서버의 작은 TCP 송신과 delayed ACK가 겹치는 약 40ms 구간을
확인했습니다. 다음은 서버 측 TCP_NODELAY 적용과 비용·회귀 검증, 남은 지연의
구간별 계측 및 여러 환경에서의 재측정입니다.

### 우선순위 1: 성능 측정과 결과 정리

- 다른 물리 머신과 여러 측정 세션에서 처리량·p95/p99·실패 재측정
- 정상·과부하·느린 클라이언트 결과와 commit별 회귀 기준값 축적
- 서버 측 TCP_NODELAY 적용 시 지연·패킷 수·CPU 비용과 단일 방·slow-client 회귀 검증
- TCP 설정 변경 뒤 남은 지연의 queue·스케줄링·클라이언트 구간별 계측
- 더 큰 부하와 방 수에서 `RoomService` mutex 경합 및 처리량 재측정
