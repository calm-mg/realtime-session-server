...
집합을 분리한 외부 대상 측정도 수행했습니다. 다음은 외부 반복의 소켓 오류
원인 확인이며, 이후 지연·mutex 병목 분석과 여러 환경에서의 재측정입니다.

### 우선순위 1: 성능 측정과 결과 정리

- 다른 물리 머신과 여러 측정 세션에서 처리량·p95/p99·실패 재측정
- 정상·과부하·느린 클라이언트 결과와 commit별 회귀 기준값 축적
- 외부 부하 반복의 서버 소켓 오류 종료 1건 원인 확인:
  [동일 조건 재현 조사](performance/2026-09-08-socket-error-investigation/README.md)와
  오류 순간 NDJSON 계측 추가, 원인은 미확정
