...
C++20과 Linux `epoll`로 구현한 실시간 세션 서버입니다. 여러 연결에서 요청이
동시에 들어오거나, 상대가 데이터를 받지 않거나, 처리 중 연결이 끊길 때의
동작을 다룹니다. 채팅방과 위치 전송은 이런 상황을 실행하고 검증하는 기능입니다.
Qt 데스크톱·콘솔 클라이언트와 TCP 부하 도구를 함께 제공합니다.

중점은 **요청 처리 순서, 과부하와 연결 종료의 정확성, 측정에 근거한 지연
개선**입니다. 아래는 `v0.1.0`에서 구현하고 검증한 내용입니다.
...
| 상황 | 처리 방식 | 상세 설명 |
| --- | --- | --- |
| 메시지가 나뉘거나 여러 개가 붙어서 도착 | 연결별 수신 버퍼에 모으고 길이 헤더로 완성된 패킷 추출 | [패킷 구성](protocol.md#패킷-구성) |
| 요청 처리나 DB 조회가 오래 걸림 | I/O, 업무 worker, DB 작업 스레드를 분리 | [전체 흐름](architecture.md#전체-흐름) |
| 여러 worker가 같은 연결의 요청을 처리 | 세션별 순서 번호로 요청과 연결 종료 이벤트를 순서대로 처리 | [WorkerPool](architecture.md#workerpool과-sessioneventhandler) |
| 요청이 처리 속도보다 빠르게 들어옴 | 입력 큐에 상한을 두고 소켓 읽기를 중지·재개 | [입력 흐름 제어](architecture.md#입력-흐름-제어) |
| 특정 클라이언트가 데이터를 받지 않음 | 세션별 송신 대기량을 제한하고 상한을 넘은 연결을 종료 | [출력 흐름](architecture.md#출력-흐름과-느린-클라이언트-격리) |
| 처리 중 연결이 끊기거나 서버가 종료됨 | 종료 이벤트를 보존하고 남은 작업과 객체 수명을 관리 | [예외 격리](architecture.md#handler-예외-격리), [종료 순서](architecture.md#종료-순서) |
| 부하가 걸릴 때 누락이나 지연이 발생 | 실제 TCP 부하로 누락·중복·지연과 서버 통계를 측정 | [부하 시나리오](benchmark.md#실제-서버-부하-시나리오) |
...
데이터 흐름과 제한값은 [서버 구조](architecture.md), 패킷·버전 협상과
