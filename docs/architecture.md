...
5. worker 결과를 outbound queue에 넣는다.
6. I/O thread가 `EPOLLOUT`을 켜고 pending write를 flush한다.
7. disconnect 또는 timeout이 발생하면 `RoomService::disconnect`로 room 상태를 정리한다.

## 향후 개선 방향

현재 구조에서 성능 개선 대상으로 보는 지점은 다음과 같습니다.

- outbound queue를 주기적으로 drain하는 구조를 `eventfd` 기반 wakeup으로 변경
