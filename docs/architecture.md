...
현재 이 종료 절차는 `TcpServer::stop()`이 호출될 때 시작합니다. 서버 실행
파일은 아직 `SIGINT`와 `SIGTERM`을 `stop()` 요청으로 연결하지 않으므로,
운영체제의 프로세스 종료 신호에서 이 절차를 사용하도록 만드는 작업이
남아 있습니다. 구체적인 추적 상태는 [알려진 문제](known-issues.md)를
참고합니다.
