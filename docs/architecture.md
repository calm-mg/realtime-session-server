...
`MessageRouter`는 연결별 선택 버전을 mutex로 보호하고 `Disconnected` 처리 시
지웁니다. 협상 전 요청과 실패한 연결의 후속 요청은 도메인·DB에 전달하지
않습니다. 협상 오류는 한 출력 메시지에 오류 바이트와 송신 후 종료 의도를
함께 담습니다. I/O 스레드는 이를 수신하면 추가 읽기를 중지하고 남은 바이트를
송신한 뒤 연결을 닫습니다. 상세 계약은 [프로토콜 문서](protocol.md)를
참고합니다.
...
설정되어 있습니다. 협상 응답이 출력 예산을 초과하는 경우에도 연결을
종료할 수 있도록, 현재 입력 세션을 닫는 빈 종료 명령 한 개는 업무 출력
예산과 별도로 허용합니다. 다른 세션 대상·바이트 첨부·중복 종료 명령과
종료 명령 뒤 추가 출력은 거절합니다.
