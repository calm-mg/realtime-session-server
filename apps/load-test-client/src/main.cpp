...
bool waitForPacket(int fd, rss::protocol::PacketCodec& codec,
                   rss::protocol::PacketType expected) {
...
void runClient(const std::string& host, std::uint16_t port, int client_index,
               int messages, ClientResult& result) {
  ...
  PacketCodec codec;
  const auto login = PacketCodec::encode(
      PacketType::LoginReq, "load-" + std::to_string(client_index));
  if (!sendAll(fd, login)) {
    ++result.failures;
    ::close(fd);
    return;
...
std::vector<std::chrono::microseconds> collectLatencies(
  ...
  for (const auto& result : results) {
    samples.insert(samples.end(), result.latencies.begin(),
...
int main(int argc, char** argv) {
  ...
  for (int i = 0; i < clients; ++i) {
    threads.emplace_back(runClient, host, port, i, messages,
  ...
  std::cout << "clients=" << clients << " messages_per_client=" << messages
            << " sent=" << sent << " failed_clients=" << failed
            << " elapsed_sec=" << elapsed
